...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func applyFilter(name: String) {
        ...
        let backgroundUIImage = UIImage(named: "imagetest")!
        let backgroundCIImage = CIImage(image: backgroundUIImage)!

        let maskFilter = CIFilter(name: "CIRadialGradient")!
        maskFilter.setValue(CIVector(x: 150, y: 150), forKey: "inputCenter")
        maskFilter.setValue(100, forKey: "inputRadius0")
        maskFilter.setValue(150, forKey: "inputRadius1")
        maskFilter.setValue(CIColor.white, forKey: "inputColor0")
        maskFilter.setValue(CIColor.black, forKey: "inputColor1")
        let maskCIImage = maskFilter.outputImage!

        
        switch filterType {
        case .additionCompositing, .colorBlendMode,
                ...
                .sourceInCompositing, .sourceOutCompositing, .sourceOverCompositing:
            filter.setValue(ciImage, forKey: kCIInputImageKey)
            filter.setValue(ciImage, forKey: kCIInputBackgroundImageKey)
        case .subtractBlendMode:
            if let foreground = UIImage(named: "CheckImage"),
               let background = UIImage(named: "imagetest"),
               let resultImage = applySubtractBlendMode(foreground: foreground, background: background) {
                imageView.image = resultImage
            }
        case .blendWithAlphaMask, .blendWithMask:
    ...
    func applySubtractBlendMode(foreground: UIImage, background: UIImage) -> UIImage? {
        guard let fgCIImage = CIImage(image: foreground),
              let bgCIImage = CIImage(image: background),
              let filter = CIFilter(name: "CISubtractBlendMode") else {
            return nil
        }
        
        filter.setValue(fgCIImage, forKey: kCIInputImageKey)
